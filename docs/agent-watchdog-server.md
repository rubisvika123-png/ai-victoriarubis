...
Сторож ловит пять разных поломок:
...
5. **Claude обновился сам** (он делает это время от времени, без спроса).
   У обновления два неприятных последствия, и сторож знает про оба:
   новая версия ставится в другую папку (`~/.local/bin`) — в скрипте ниже
   эта папка сразу прописана в путях, иначе агент не запустится с ошибкой
   `claude: No such file or directory`; и связь с Telegram с версии
   2.1.252 работает внутри самого Claude, отдельного процесса-моста больше
   нет — старая проверка связи принимала живого агента за мёртвого и
   перезапускала его по кругу (в журнале `NO PULSE` каждую минуту).
   В `pulse()` ниже учтены оба варианта: и старый отдельный мост, и новый
   встроенный.
...
# ~/.local/bin — туда Claude Code ставит себя при самообновлении, ~/.bun/bin —
...
pulse() {
  ...
  [ -n "$pane_pid" ] || return 0
  # До версии 2.1.252 мост в Telegram был отдельным процессом (bun server.ts),
  # начиная с неё он работает внутри самого claude. Признак живой связи —
  # любой из двух, иначе сторож убивал бы здорового агента каждую минуту.
...
Проверка «не душит ли сторож живого агента»: через пару минут спокойной
работы в `~/watchdog.log` не должно быть повторяющихся строк
`NO PULSE ... restarting`, а `tmux ls` дважды с промежутком в две минуты
должен показывать одно и то же время создания сессии. Если строки `NO PULSE`
